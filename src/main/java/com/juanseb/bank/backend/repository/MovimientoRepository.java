...
public interface MovimientoRepository extends JpaRepository<Movimiento, Long> {
	...
	@Query("SELECT m FROM Movimiento m WHERE m.tarjeta.id= :idTarjeta AND m.usuario.id =:idUsuario")
	List<Movimiento> obtenerMovimientosDeTarjetaByUsuario(@Param("idTarjeta")Long idTarjeta, @Param("idUsuario")Long idUsuario);

	@Query("SELECT m FROM Movimiento m WHERE m.cuenta.id= :idCuenta AND m.usuario.id =:idUsuario ORDER BY m.fecha DESC")
	...
	@Query("select m from Movimiento m where m.cuenta.id = :idCuenta AND m.usuario.id =:idUsuario AND m.fecha BETWEEN :fechaInit AND :fechaFin ORDER BY m.fecha")
